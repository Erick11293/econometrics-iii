...
def compute_CCV(df, u, nmx, pk):
    ...
    # compute avg Z
    Zavg = float(np.sum(df['u']==1-u))/df.shape[0]
...
def get_sample(df, state_df, nmx):
    w1 = df[nmx]==1
    w0 = df[nmx]==0
...
def boostrap_taus(B, df, state_df, nmx, model):
    ...
    taus_FE = []
    for b in range(B):
        sample = get_sample(df, state_df, nmx)
...
def sample_pq(pk, qk, df):
    ...
    for m in df['state'].unique():
        sample_m = np.random.binomial(1, qk)
        if sample_m == 1:
            ind_m = df['state']==m
            df_m = df[df['state']==m]
            units = np.random.binomial(1, pk, size=df_m.shape[0])
